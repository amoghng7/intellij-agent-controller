 ...
 * This is the high-level entry point used by the CLI module.
 */
fun startMcpServer(
    transport: String,
    mcpPort: Int = 3000,
    robotHost: String = "127.0.0.1",
    robotPort: Int = 8580
): Unit = runBlocking {
    val server = createMcpServer(ConnectionManager(), robotHost, robotPort)
    when (transport) {
        "sse" -> runSse(server, mcpPort)
...
suspend fun runStdio(server: Server) {
...
fun runSse(server: Server, port: Int) {
